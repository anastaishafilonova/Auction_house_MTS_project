...
public class ProductCreateGateway {
    ...
    public static void createProduct(long productId, Request.RequestToCreateProduct request) {
        try {
            ProductRequestToCreate productRequestToCreate = new ProductRequestToCreate(productId, request.getStartTime(), request.getFinishTime(), request.getMinBet(), request.getPrice(), request.getSellerId(), -1);
